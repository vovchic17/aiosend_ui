...
export function DeleteInvoiceCode({
  invoiceId,
  isRunning,
  canRun,
  output,
      ...
      <div className="overflow-x-auto px-5 py-5 pr-14">
        <div className="min-w-[520px] space-y-1">
          ...
          <CodeLine number={4}>
            ...
            <span className="text-syntax-built-in">=</span>{" "}
            <span>cp.</span>
            <span className="text-syntax-built-in">delete_invoice</span>
            <span>(</span>
          </CodeLine>
          <CodeLine
            number={5}
            parameter="invoice_id"
            onParameterEnter={() => onParameterEnter("invoice_id")}
            onParameterLeave={onParameterLeave}
          >
            <span className="ml-8 text-syntax-parameter">invoice_id=</span>
            <CodeInput
              value={invoiceId}
              onChange={onInvoiceIdChange}
              ariaLabel={copy.invoiceIdInputLabel}
              type="number"
              tone="number"
              className="w-40"
            />
          ...
          <CodeLine number={8}>
            ...
            <span>(</span>
            <span className="text-syntax-string">&quot;Invoice deleted:&quot;</span>
            <span>,</span>{" "}
            <span>status</span>
            <span>)</span>
          </CodeLine>
