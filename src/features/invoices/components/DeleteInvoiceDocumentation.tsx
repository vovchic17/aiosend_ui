...
import type { DeleteInvoiceParameterName } from "../types";

type DeleteInvoiceDocumentationProps = {
  activeParameter: DeleteInvoiceParameterName | null;
  copy: Dictionary["invoices"]["playground"];
  onParameterEnter: (parameter: DeleteInvoiceParameterName) => void;
  onParameterLeave: () => void;
};

export function DeleteInvoiceDocumentation({
  activeParameter,
  copy,
  onParameterEnter,
  onParameterLeave,
}: DeleteInvoiceDocumentationProps) {
  const isActive = activeParameter === "invoice_id";

  return (
    <div className="flex h-full flex-col gap-5 py-1 text-code-m text-content">
      <div className="code-signature">
        <a
          href="https://aiosend.readthedocs.io/en/latest/api/methods.html#aiosend.CryptoPay.delete_invoice"
          target="_blank"
          rel="noreferrer"
          className="text-syntax-built-in hover:underline"
        >
          CryptoPay.delete_invoice
        </a>
        <span>(</span>
        <span className="text-syntax-parameter">invoice_id</span>
        <span>)</span>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <ul className="m-0 list-none p-0">
          <li
            ...
            onMouseEnter={() => onParameterEnter("invoice_id")}
            onMouseLeave={onParameterLeave}
          >
            <span className="text-code-m text-syntax-parameter">invoice_id</span>
            <span className="font-body text-body">
              <span className="italic text-content-muted"> (int)</span>
              <span> — {copy.deleteDocs.invoiceId}</span>
            </span>
          </li>
        </ul>
      </div>
