...
type CreateCheckCodeProps = {
  ...
  onPinToUserIdChange: (value: string) => void;
  onPinToUsernameChange: (value: string) => void;
...
export function CreateCheckCode({
  amount,
  asset,
  pinToUserId,
  pinToUsername,
  ...
  onAssetChange,
  onPinToUserIdChange,
  onPinToUsernameChange,
  onParameterEnter,
  onParameterLeave,
  onRun,
  onCopy,
}: CreateCheckCodeProps) {
      ...
      <div className="overflow-x-auto px-5 py-5 pr-14">
        <div className="min-w-[600px] space-y-1">
          ...
          <CodeLine number={4}>
            ...
            <span className="text-syntax-built-in">=</span>{" "}
            <span>cp.</span>
            <span className="text-syntax-built-in">create_check</span>
            <span>(</span>
          </CodeLine>
          <CodeLine
            number={5}
          ...
          <CodeLine
            number={7}
            parameter="pin_to_user_id"
            onParameterEnter={() => onParameterEnter("pin_to_user_id")}
            onParameterLeave={onParameterLeave}
          >
            <span className="ml-8 text-syntax-parameter">pin_to_user_id=</span>
            <CodeInput
              value={pinToUserId}
              onChange={onPinToUserIdChange}
              ariaLabel={copy.pinToUserIdInputLabel}
              type="number"
              tone="number"
              className="w-40"
            />
          ...
          <CodeLine
            number={8}
            parameter="pin_to_username"
            onParameterEnter={() => onParameterEnter("pin_to_username")}
            onParameterLeave={onParameterLeave}
          >
            <span className="ml-8 text-syntax-parameter">pin_to_username=</span>
            <span className="text-syntax-string">&quot;</span>
            <CodeInput
              value={pinToUsername}
              onChange={onPinToUsernameChange}
              ariaLabel={copy.pinToUsernameInputLabel}
              className="w-48"
            />
            <span className="text-syntax-string">&quot;</span>
            <span>,</span>
          ...
          <CodeLine number={11}>
            ...
            <span>(</span>
            <span className="text-syntax-string">&quot;Check URL:&quot;</span>
            <span>,</span>{" "}
            <span>check.bot_check_url</span>
            <span>)</span>
          </CodeLine>
        </div>
      </div>
